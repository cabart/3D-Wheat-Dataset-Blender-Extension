...
if "--" in sys.argv:
    ...
    parser.add_argument("-d", "--density", type=int, nargs="+", required=True)
    parser.add_argument("-g", "--growth", type=int, nargs="+", required=True)
    ...
    general_props = bpy.context.scene.PlantProps
    camera_props = bpy.context.scene.CameraRenderProps

    # Load parameters from the config file
    config = load_config(args.config_file)

    if "iteration_step" in config["plants"]:
        print("Iteration step parameter is being ignored for this experiment")
    if "canopy_distance_x" in config["plants"]:
        print("Canopy distance x parameter is being ignored for this experiment")
    if "canopy_distance_y" in config["plants"]:
        print("Canopy distance y parameter is being ignored for this experiment")

    iteration_steps = args.growth
    densities = args.density

    general_props.derivation_length = max(iteration_steps)
    general_props.canopy_seed = config["plants"]["canopy_seed"]
    general_props.model = config["plants"]["model"]
    general_props.canopy_plants_x = config["plants"]["canopy_plants_x"]
    general_props.canopy_plants_y = config["plants"]["canopy_plants_y"]
    ...
    # For each density and growth stage, render the images
    for density in densities:
        for iteration_step in iteration_steps:
            general_props.iteration_step = iteration_step
            general_props.canopy_distance_x = density
            general_props.canopy_distance_y = density

            base_path = args.save_path
            camera_props.save_path = os.path.join(
                base_path, f"density_{density}_growth_{iteration_step}"
            )
            ...
            config["rendering"]["canopy_distance_x"] = density
            config["rendering"]["canopy_distance_y"] = density
            config["rendering"]["iteration_step"] = iteration_step
            config["rendering"]["derivation_length"] = max(iteration_steps)
            with open(
