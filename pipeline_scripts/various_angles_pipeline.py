...
if "--" in sys.argv:
    ...
    parser.add_argument("-a", "--angles", type=int, nargs="+", required=True)
    ...
    general_props.derivation_length = config["plants"]["iteration_step"]
    ...
    general_props.canopy_distance_x = config["plants"]["canopy_distance_x"]
    general_props.canopy_distance_y = config["plants"]["canopy_distance_y"]
    ...
    general_props.iteration_step = config["plants"]["iteration_step"]

    # For each number of train images, render the images
    for current_angle in args.angles:
        camera_props.cap_angle_train = current_angle

        base_path = args.save_path
        camera_props.save_path = os.path.join(base_path, f"train_angle_{current_angle}")
        ...
        config["rendering"]["cap_angle_train"] = current_angle
        with open(
